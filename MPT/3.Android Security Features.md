Kotlin and Java are the two primary languages used to develop Android applications. The Android SDK tools compile application source code along with resource files and assets into an Android Package (APK). An APK is an archive file with a `.apk` extension that contains all the components needed to install and run an Android app, including compiled bytecode (`.dex`), manifest metadata, resources, and native libraries.

### [^1]Linux Kernel Security
[^1]: Each Android application runs within its own isolated security sandbox, enforced by the underlying Linux-based architecture. This sandboxing model is supported by several core Android security features:
	
	Security Features
	- Android is a multi-user Linux system where each application is treated as a separate user.
	- By default, the system assigns each app a unique Linux user ID (UID). This UID is used by the system for access control, but is not exposed to the app itself.
	- File system permissions ensure that only the app assigned a particular UID can access its own files.
	- Each app runs in its own process, and each process runs in a separate instance of the Android Runtime (ART) virtual machine, ensuring memory isolation.
	- The system launches the app's process as needed and terminates it when no longer required or when reclaiming system resources.
	- Android enforces the principle of least privilege, meaning apps only receive the permissions necessary to perform their core functionality. Additional privileges must be explicitly declared in the app's manifest and approved by the user (or system, depending on the API level).
	


## 1. Linux Kernel Security
Android is built on a modified Linux kernel, which provides:

- Process isolation  
- User/group IDs for each app  
- Permissions and sandboxing  
- Memory management protection  
- SElinux enforcement  


### Importance for Pentesters

- Privilege escalation attacks target kernel exploits. 
- Rooting tools often bypass kernel-level restrictions. 
- SELinux misconfigurations may expose attack paths


## 2. Application Sandbox

- Each app runs in its own Linux User ID (UID).
- Separate process & dedicated storage location.
- Prevents unauthorized access to other apps’ data.
- Enforced by:
- Linux kernel permissions
- SELinux (from Android 4.3+)

 ### Security Implication in Pentesting:

==Misconfigured components (exported activities/services) may break sandbox isolation.==


```
                    ANDROID SYSTEM
    ┌──────────────────────────────────────────┐
    │                                          │
    │   ┌──────────┐  ┌──────────┐  ┌──────────┐
    │   │  APP A   │  │  APP B   │  │  APP C   │
    │   │  UID:    │  │  UID:    │  │  UID:    │
    │   │  10001   │  │  10002   │  │  10003   │
    │   │          │  │          │  │          │
    │   │ /data/   │  │ /data/   │  │ /data/   │
    │   │ data/    │  │ data/    │  │ data/    │
    │   │ com.a    │  │ com.b    │  │ com.c    │
    │   │          │  │          │  │          │
    │   └────┬─────┘  └────┬─────┘  └────┬─────┘
    │        │             │             │
    │        ▼             ▼             ▼
    │   ┌──────────────────────────────────────┐
    │   │         LINUX KERNEL                 │
    │   │   Process Isolation & Permissions    │
    │   └──────────────────────────────────────┘
    │                                          │
    └──────────────────────────────────────────┘
```


#### Sandbox Components

| Component             | Description                           |
| --------------------- | ------------------------------------- |
| Unique UID            | Each app gets UID starting from 10000 |
| Process Isolation     | Separate process space per app        |
| File System Isolation | Private /data/data/<package>/         |
| Memory Isolation      | Cannot read other app's memory        |
| SELinux Context       | Mandatory access control labels       |

#### File System Permissions
```
# Check app directory permissions
adb shell ls -la /data/data/

drwx------ u0_a123 u0_a123  com.example.app1
drwx------ u0_a124 u0_a124  com.example.app2
drwx------ u0_a125 u0_a125  com.example.app3

# u0_a123 = UID 10123
# Only owner can read/write/execute (rwx------)
```

#### Security Implications for Pentesters

┌─────────────────────────────────────────────────────────────┐
│           SANDBOX BYPASS TECHNIQUES                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ► Root Access                                              │
│    └── Bypasses all sandbox restrictions                    │
│                                                             │
│  ► Shared UID (android:sharedUserId)                        │
│    └── Apps with same signature can share UID               │
│                                                             │
│  ► Exported Components                                      │
│    └── Exposed activities/services/providers                │
│                                                             │
│  ► World-Readable Files                                     │
│    └── Misconfigured file permissions                       │
│                                                             │
│  ► Content Provider Vulnerabilities                         │
│    └── SQL injection, path traversal                        │
│                                                             │
│  ► Backup Extraction                                        │
│    └── android:allowBackup="true"                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘

## 3. Permission Model

#### Permission Types

┌─────────────────────────────────────────────────────────────┐
│                 PERMISSION CATEGORIES                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  NORMAL PERMISSIONS                                         │
│  └── Automatically granted at install                       │
│  └── Low risk to user privacy                               │
│  └── Examples: INTERNET, BLUETOOTH, VIBRATE                 │
│                                                             │
│  DANGEROUS PERMISSIONS                                      │
│  └── Must be explicitly granted by user                     │
│  └── Access sensitive data or resources                     │
│  └── Examples: CAMERA, CONTACTS, LOCATION                   │
│                                                             │
│  SIGNATURE PERMISSIONS                                      │
│  └── Granted only if same signing certificate               │
│  └── Used for inter-app communication (same developer)      │
│                                                             │
│  SIGNATURE|PRIVILEGED PERMISSIONS                           │
│  └── For system apps in /system/priv-app/                   │
│  └── Highest privilege level                                │
│                                                             │
└─────────────────────────────────────────────────────────────┘

#### Runtime Permissions

NEW WAY (Android 6.0+):
───────────────────────
Install App
    │
    ▼
App installed (no permissions yet)
    │
    ▼
User opens app and tries to take photo
    │
    ▼
┌────────────────────────────┐
│  Allow Camera access?      │
│  [DENY]    [ALLOW]         │
└────────────────────────────┘
    │
    ▼
User tries to share to contacts
    │
    ▼
┌────────────────────────────┐
│  Allow Contacts access?    │
│  [DENY]    [ALLOW]         │
└────────────────────────────┘

Each permission asked SEPARATELY
when app needs it

#### Special Permission Options (Android 11+)
When app asks for permission, you see:

┌─────────────────────────────────────┐
│                                     │
│  Allow "App" to access location?    │
│                                     │
│  ○ Allow all the time              │
│  ○ Allow only while using app      │
│  ○ Ask every time                  │
│  ○ Deny                            │
│                                     │
└─────────────────────────────────────┘

#### Signature Permission
SIGNATURE PERMISSION:
─────────────────────

Only granted if two apps have SAME developer signature

Example:
┌──────────────┐     ┌──────────────┐
│   App A      │     │   App B      │
│  (Facebook)  │────▶│ (Messenger)  │
│              │     │              │
│ Signed by:   │     │ Signed by:   │
│  Facebook    │     │  Facebook    │
└──────────────┘     └──────────────┘
      │                     │
      └──────────┬──────────┘
                 ▼
        SAME SIGNATURE
        Can share data via
        signature permission


#### Privileged Permission

PRIVILEGED PERMISSION:
──────────────────────

Only for system apps installed in:
/system/priv-app/

Examples:
• Install/Uninstall apps
• Modify system settings
• Access all user data

Regular apps CANNOT get these permissions

### Checking Permissions with ADB

```
# List all permissions of an app
adb shell dumpsys package com.example.app | grep permission

# See granted permissions
adb shell dumpsys package com.example.app | grep "granted=true"

# See denied permissions  
adb shell dumpsys package com.example.app | grep "granted=false"
```

#### Grant Permission via ADB
```
# Grant a permission
adb shell pm grant com.example.app android.permission.CAMERA

# Revoke a permission
adb shell pm revoke com.example.app android.permission.CAMERA
```

### Where Permissions Are Declared

==Apps declare permissions in AndroidManifest.xml:==

```
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    package="com.example.myapp">
    
    <!-- Normal Permissions (auto-granted) -->
    <uses-permission android:name="android.permission.INTERNET"/>
    <uses-permission android:name="android.permission.VIBRATE"/>
    
    <!-- Dangerous Permissions (user must approve) -->
    <uses-permission android:name="android.permission.CAMERA"/>
    <uses-permission android:name="android.permission.READ_CONTACTS"/>
    <uses-permission android:name="android.permission.ACCESS_FINE_LOCATION"/>
    
</manifest>
```


## 4.SELinux (Security-Enhanced Linux)

Extra security layer on top of normal Linux permissions

NORMAL LINUX SECURITY:                                    │
│   "Can this USER access this FILE?"                         │
│                                                             │
│   SELINUX SECURITY:                                         │
│   "Can this PROCESS of this TYPE access this RESOURCE       │
│    of this TYPE in this WAY?"                               │
│                                                             │
│   SELinux adds MORE rules on top of normal permissions 


#### Why Does Android Need SELinux?

┌─────────────────────────────────────────────────────────────┐
│                                                             │
│   PROBLEM WITHOUT SELINUX:                                  │
│                                                             │
│   If attacker gets root access:                             │
│   └── Can access EVERYTHING                                 │
│   └── Can read all files                                    │
│   └── Can modify system                                     │
│   └── Can install malware                                   │
│   └── Complete device compromise                            │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   WITH SELINUX:                                             │
│                                                             │
│   Even if attacker gets root access:                        │
│   └── Still LIMITED by SELinux policies                     │
│   └── Cannot access protected resources                     │
│   └── Damage is CONTAINED                                   │
│   └── Defense in depth                                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘


#### Two Types of Access Control

##### 1.DAC - Discretionary Access Control
```
DAC = Normal Linux Permissions

"Owner DECIDES who can access"

Example:
-rw-r--r-- user1 group1 file.txt

Owner (user1) can change these permissions
Owner has DISCRETION (choice)
```

##### 2.MAC - Mandatory Access Control
```
MAC = SELinux Permissions

"SYSTEM POLICY decides who can access"

Example:
Even if file permissions say "everyone can read"
SELinux policy might say "only browser can read"

Owner CANNOT override SELinux policy
It is MANDATORY (required)
```

#### SELinux Modes
┌─────────────────────────────────────────────────────────────┐
│                   SELINUX MODES                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   1. DISABLED                                               │
│      └── SELinux is completely OFF                          │
│      └── No protection                                      │
│      └── Rarely used in Android                             │
│                                                             │
│   2. PERMISSIVE                                             │
│      └── SELinux is ON but only LOGS violations             │
│      └── Does NOT block anything                            │
│      └── Used for testing/debugging                         │
│                                                             │
│   3. ENFORCING                                              │
│      └── SELinux is ON and BLOCKS violations                │
│      └── Full protection                                    │
│      └── Default for production Android                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘


##### SELinux Labels (Context)

┌─────────────────────────────────────────────────────────────┐
│                                                             │
│   SELinux Label Format:                                     │
│                                                             │
│   user : role : type : level                                │
│     │      │      │       │                                 │
│     │      │      │       └── Security level (MLS/MCS)      │
│     │      │      └── TYPE (most important!)                │
│     │      └── Role (what can be done)                      │
│     └── User (SELinux user, not Linux user)                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘

Example:
u:r:untrusted_app:s0:c512,c768

u            = user
r            = role
untrusted_app = type (THIS IS THE KEY PART)
s0:c512,c768 = level


## 5: Application Signing

- Every Android app MUST be signed before installation

- Signing = Developer's digital signature on the app

#### Why Signing Matters
┌─────────────────────────────────────────────────────────────┐
│                  PURPOSE OF SIGNING                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. IDENTITY                                                │
│     └── Proves who made the app                             │
│                                                             │
│  2. INTEGRITY                                               │
│     └── Proves app wasn't modified                          │
│                                                             │
│  3. TRUST                                                   │
│     └── Updates must use same key                           │
│     └── Prevents fake updates                               │
│                                                             │
│  4. PERMISSIONS                                             │
│     └── Signature-level permissions                         │
│     └── Apps with same signature can share data             │
│                                                             │
└─────────────────────────────────────────────────────────────┘


#### How Signing Works
DEVELOPER SIDE:
───────────────

┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│   APK File   │───▶│  Private Key │───▶│  Signed APK  │
│  (unsigned)  │    │  (secret)    │    │              │
└──────────────┘    └──────────────┘    └──────────────┘


ANDROID SIDE:
─────────────

┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  Signed APK  │───▶│  Public Key  │───▶│  Verified!   │
│              │    │ (in the APK) │    │  Install OK  │
└──────────────┘    └──────────────┘    └──────────────┘

#### Signing Schemes

┌─────────────────────────────────────────────────────────────┐
│                   APK SIGNING SCHEMES                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  v1 - JAR Signing (Old)                                     │
│  └── Android 1.0+                                           │
│  └── Signs individual files                                 │
│  └── Stored in META-INF/ folder                             │
│                                                             │
│  v2 - APK Signature Scheme (Better)                         │
│  └── Android 7.0+ (API 24)                                  │
│  └── Signs entire APK as one block                          │
│  └── Faster verification                                    │
│                                                             │
│  v3 - Key Rotation Support                                  │
│  └── Android 9.0+ (API 28)                                  │
│  └── Allows changing signing key                            │
│                                                             │
│  v4 - Streaming Install                                     │
│  └── Android 11+ (API 30)                                   │
│  └── For incremental installs                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘

## 6.Verified Boot

- Verified Boot = Checks if device software is tampered
- Ensures only TRUSTED software runs on device

#### Boot Chain
┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐
│ Boot ROM │──►│Bootloader│──►│  Kernel  │──►│  System  │
│ (Chip)   │   │          │   │          │   │          │
└──────────┘   └──────────┘   └──────────┘   └──────────┘
     │              │              │              │
     └───── Each step VERIFIES next step ────────┘


Verified Boot States
Color                	Meaning
🟢 Green        	Fully verified, locked
🟡 Yellow	        Custom key, locked
🟠 Orange	        Bootloader unlocked
🔴 Red	            Verification failed


## 7.Encryption
Encryption = Data stored in unreadable format

Without correct key/password → Cannot read data

Protects data if device is:
├── Lost
├── Stolen
└── Seized


#### Two Types of Encryption
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  FDE (Full Disk Encryption)                                 │
│  └── Old method (Android 4.4 - 9)                           │
│  └── Encrypts ENTIRE /data partition                        │
│  └── One key for everything                                 │
│  └── Must decrypt at boot                                   │
│                                                             │
│  FBE (File-Based Encryption)                                │
│  └── New method (Android 7.0+, default in 10+)              │
│  └── Each file encrypted separately                         │
│  └── Different keys for different data                      │
│  └── Allows Direct Boot                                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘

##### FBE Storage Types
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  CE (Credential Encrypted)                                  │
│  └── Unlocked AFTER user enters PIN/password                │
│  └── Contains most app data                                 │
│  └── Location: /data/data/                                  │
│                                                             │
│  DE (Device Encrypted)                                      │
│  └── Available IMMEDIATELY after boot                       │
│  └── Before user unlocks                                    │
│  └── Alarms, accessibility, phone calls work                │
│  └── Location: /data/user_de/                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘


## 8.Android Keystore System
Keystore = Secure container for cryptographic keys

Keys stored here are:
├── Hard to extract
├── Can require authentication to use
└── May be hardware-protected

#### Types of Keystore
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  SOFTWARE KEYSTORE                                          │
│  └── Keys in encrypted file                                 │
│  └── Protected by Android OS                                │
│  └── Can be extracted with root + effort                    │
│                                                             │
│  HARDWARE-BACKED KEYSTORE (TEE)                             │
│  └── Keys in Trusted Execution Environment                  │
│  └── Separate secure processor                              │
│  └── Keys NEVER leave hardware                              │
│  └── Very hard to extract                                   │
│                                                             │
│  STRONGBOX (Android 9+)                                     │
│  └── Dedicated security chip                                │
│  └── Tamper-resistant hardware                              │
│  └── Highest security level                                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘

## 9.SafetyNet / Play Integrity API

SafetyNet = Google's device integrity check

Tells app if device is:
├── Rooted
├── Has unlocked bootloader
├── Running custom ROM
├── Tampered with
└── An emulator

##### SafetyNet vs Play Integrity API
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  SAFETYNET (Old - Being Deprecated)                         │
│  └── Introduced 2014                                        │
│  └── Being replaced by Play Integrity                       │
│  └── Still used by many apps                                │
│                                                             │
│  PLAY INTEGRITY API (New - 2022+)                           │
│  └── Replacement for SafetyNet                              │
│  └── More detailed verdicts                                 │
│  └── Harder to bypass                                       │
│  └── Will be standard going forward                         │
│                                                             │
└─────────────────────────────────────────────────────────────┘


#### Play Integrity Verdicts
┌─────────────────────────────────────────────────────────────┐
│             PLAY INTEGRITY VERDICTS                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  MEETS_DEVICE_INTEGRITY                                     │
│  └── App running on genuine Android device                  │
│  └── Passes Google Play checks                              │
│                                                             │
│  MEETS_BASIC_INTEGRITY                                      │
│  └── Device passes basic checks                             │
│  └── May be rooted but passes some tests                    │
│                                                             │
│  MEETS_STRONG_INTEGRITY                                     │
│  └── Hardware-backed security confirmed                     │
│  └── Strongest check                                        │
│                                                             │
│  NO_INTEGRITY                                               │
│  └── Failed all checks                                      │
│  └── Emulator, rooted, or heavily modified                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘


## 10.Network Security Configuration
#### Network Security Config

Controls how app makes network connections

Defines:
├── Trust which certificates
├── Allow HTTP or only HTTPS
├── Certificate pinning
└── Debug settings

**Location**
```
res/xml/network_security_config.xml

Referenced in AndroidManifest.xml:
android:networkSecurityConfig="@xml/network_security_config"
```


## 11.Biometric Authentication
Biometric = Using body features to authenticate

Types:
├── Fingerprint
├── Face recognition
└── Iris scan

#### Biometric Security Classes (Android 11+)
┌─────────────────────────────────────────────────────────────┐
│              BIOMETRIC CLASSES                              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  CLASS 3 (Strong)                                           │
│  └── Hardware-backed biometrics                             │
│  └── Can unlock Keystore keys                               │
│  └── High security                                          │
│  └── Example: Most fingerprint sensors                      │
│                                                             │
│  CLASS 2 (Weak)                                             │
│  └── Software-based                                         │
│  └── Cannot unlock crypto keys                              │
│  └── Medium security                                        │
│  └── Example: Some face unlock                              │
│                                                             │
│  CLASS 1 (Convenience)                                      │
│  └── Lowest security                                        │
│  └── Easy to spoof                                          │
│                                                             │
└─────────────────────────────────────────────────────────────┘