...
![[Pasted image 20251227204008.png]]
...
Each app:
1. ├── Gets unique Linux User ID (UID)
2. ├── Runs in separate process
3. ├── Has private data directory
4. ├── Cannot access other apps' data
...
| ------------------- | --------- | --------------------------------------------------------------------------------------------- |
| VERIFIED            | 🟢 Green  | Fully verified boot chain. All signatures valid. Bootloader locked. OEM certificate used.     |
| SELF-SIGNED         | 🟡 Yellow | Custom key verification. Bootloader locked. User-added certificate. Warning shown at boot.    |
| UNLOCKED            | 🟠 Orange | Bootloader unlocked. Verification may be skipped. Warning shown at boot. Custom ROMs allowed. |
| VERIFICATION FAILED | 🔴 Red    | Corrupted boot chain. May refuse to boot. Or boot with severe warnings. Data may be wiped.    |
...
┌────────────────────────────────────────┐
│        KEYSTORE API (Java)              │
│   (KeyStore, KeyGenerator, Cipher)      │
└────────────────────┬───────────────────┘
                     │
                     ▼
┌────────────────────────────────────────┐
│         KEYSTORE SERVICE               │
│    (keystore2 daemon - Android 12+)    │
└────────────────────┬───────────────────┘
                     │
       ┌─────────────┴─────────────┐
       ▼                           ▼
┌──────────────┐          ┌──────────────┐
│  TEE-backed  │          │  StrongBox   │
│  Keymaster   │          │  Keymaster   │
└──────────────┘          └──────────────┘
```
