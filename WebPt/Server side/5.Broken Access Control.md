...
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   VULNERABILITY TYPES                    OUTCOMES/IMPACTS                │
│   (Attack Vectors)                       (What attacker achieves)        │
│                                                                          │
│   ┌─────────────────────────────┐        ┌────────────────────────────┐  │
│   │ IDOR                        │───┐    │                            │  │
│   ├─────────────────────────────┤   │    │  Horizontal Privilege      │  │
│   │ Forced Browsing             │───┤    │  Escalation                │  │
│   ├─────────────────────────────┤   ├───▶│  (User A → User B data)    │  │
...
│   ├─────────────────────────────┤   │    │  Vertical Privilege        │  │
│   │ Cookie Manipulation         │───┼───▶│  Escalation                │  │
│   ├─────────────────────────────┤   │    │  (User → Admin)            │  │
...
│   │ Access Control              │   │    ┌────────────────────────────┐  │
│   ├─────────────────────────────┤   │    │                            │  │
│   │ CORS Misconfiguration       │───┤    │  Unauthorized Access       │  │
│   ├─────────────────────────────┤   ├───▶│  - Data Theft              │  │
│   │ Metadata/Header Manipulation│───┤    │  - File Access             │  │
│   ├─────────────────────────────┤   │    │  - Data Modification       │  │
│ x │ BOLA (API-specific IDOR)    │───┤    │  - Data Deletion           │  │
│   ├─────────────────────────────┤   │    │                            │  │
│ x │ BFLA (API Function-Level)   │───┘    └────────────────────────────┘  │
│   └─────────────────────────────┘                                        │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│   │                             │   │    │                            │  │
│   │ 5. Open Redirect            │───┼───▶│  Vertical Privilege        │  │
│   │                             │   │    │  Escalation                │  │
│   │ 6. Missing Function-Level   │   │    │  (User → Admin)            │  │
│   │    Access Control           │───┤    │                            │  │
│   │                             │   │    └────────────────────────────┘  │
...
### 1. Forced Browsing (Missing Function Level Access Control)

**How it happens:** Developers often hide sensitive buttons (like "Admin Panel" or "Delete User") from the interface for normal users. They assume: _"If the user can't see the button, they can't click it."_

- **The Technical Failure:** The developer put the security check on the **Front-end** (HTML/UI), but forgot to put the check on the **Back-end** (API/Server).
    
- **The Attack:**
    
    1. The attacker guesses the URL (e.g., `/admin/deleteUser`).
        
    2. They type it directly into the browser address bar.
        
    3. The server receives the request. Since there is no code checking "Is this user an Admin?" on that specific page, it loads successfully.
        

### 2. Insecure Direct Object References (IDOR)

**How it happens:** Databases identify records using unique numbers (Primary Keys), like `User_ID=101`, `File_ID=500`. To retrieve data, the application asks the user "Which ID do you want?"

- **The Technical Failure:** The application takes the ID provided by the user and puts it directly into a database query without checking **ownership**.
    
- **The Code Logic (Vulnerable):** `SELECT * FROM invoices WHERE invoice_id = <User_Input>`
    
- **The Code Logic (Secure):** `SELECT * FROM invoices WHERE invoice_id = <User_Input> AND owner_id = <Current_User_Session>`
    
- **The Attack:** You simply change the number in the URL or API call from `100` to `101`, and the database obediently returns the record for ID 101.
